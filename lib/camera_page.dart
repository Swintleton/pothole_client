...
class _CameraPageState extends State<CameraPage> {
  ...
  Timer? timer;
  Timer? confirmationPoller; // Timer for polling detections

  @override
  void initState() {
    super.initState();
    initializeCamera();
    startConfirmationPolling(); // Start polling for pending detections
  }
  ...
  void dispose() {
    controller.dispose();
    timer?.cancel();
    confirmationPoller?.cancel(); // Stop polling when disposing the widget
    super.dispose();
  }
  ...
  // Poll the server for pending detections every 4 seconds
  void startConfirmationPolling() {
    confirmationPoller = Timer.periodic(const Duration(seconds: 4), (Timer t) {
        pollForDetectionConfirmation();
    });
  }

  Future<void> pollForDetectionConfirmation() async {
    final authToken = await AuthHelper.getAuthToken();

    final response = await http.get(
      Uri.parse('http://192.168.0.115:5000/get_detection_confirmation'),
      headers: {
    ...
    if (response.statusCode == 200) {
      final detectionData = jsonDecode(response.body);
      if (detectionData.isNotEmpty) {
        final filename = detectionData['filename'];
        final imageUrl = 'http://192.168.0.115:5000/confirmed/$filename';
        
        // Show confirmation
        await showConfirmationDialog(filename, imageUrl);
      }
    } else if (response.statusCode == 401) {
      // Invalid token
      AuthHelper.logout(context, mounted);
    } else {
      print('Failed to fetch detection confirmation');
    }
  }
  ...
  Future<void> showConfirmationDialog(String filename, String imageUrl) async {
    ...
    // Call the /confirm endpoint to send the user's confirmation response
    await sendConfirmationResponse(filename, confirmed ?? false);
  ...
  Future<void> sendConfirmationResponse(String filename, bool confirmed) async {
    // Remove the "_detected" suffix from filename if present
    final cleanedFilename = filename.replaceAll('_detected.jpg', '.jpg');

    final authToken = await AuthHelper.getAuthToken();

    final response = await http.post(
      Uri.parse('http://192.168.0.115:5000/confirm'),
      headers: {
        'Authorization': '$authToken',
